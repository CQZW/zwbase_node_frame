...
class ZWBaseCtr {
     ...
     * 新实例不会调用 srvStartOK,srvWillStart
     * @param {*} calssname
     * @memberof ZWBaseCtr
     */
    ...
    {
        return this.getSessionClass().loadSession( sessionid );
    }
    ...
    async loadAllSessionToCache()
    {
        ...
        let loadcount = await this.getSessionClass().loadAllSessionToCache( max,cache );
    ...
    startRuningJob( time_ms = 10000 ) {
        ...
        this.getSrv().robMachineLock().then( (machine_lock)=>{
            setTimeout(()=>{this.job_runing(machine_lock)}, time_ms);
        });
    }
     ...
     * @param {*} machine_lock-机器锁状态
     * @returns
     * @memberof ZWBaseCtr
     */
    async job_runing( machine_lock ) {
        this.getSrv().robMachineLock().then( (machine_lock)=>{
            setTimeout( ()=>{this.job_runing(machine_lock)}, this._time_ms );
        });
        return Promise.resolve();
    }
