   ...
   ctrConfig() {
    ...
    async checkParam(param) {
        ...
        do
        {
            ...
            if( this.getSrv().ctrGetSrvCfgInfo().need_session )
            {
    ...
    {
        //这里使用缓存,并且没有使用maxage,有session自己判断的
        ...
        if( param.sessionid )
        {
            ...
            v = await this.getSessionSampleObj().loadSession( param.sessionid );
            if( v )
            {
                if( !v.isVaild() )
                {
     ...
     * **** **** **** **** **** **** **** **** **** **** ***
     * 重载session实现方式:
     * 1.底层框架自带的,cookie之类,修改 getReqParams 和 willSend 即可
     * 2.控制器继承修改 loadAllSessionToCache , getSessionSampleObj , makeSession ,
     * 3.ZWSession 类继承修改.主要是通过继承 ZWSession 实现自定义session功能
     * * **** **** **** **** **** **** **** **** **** **** ***
     */

    /**
     * 
     * 返回session范例对象,然后用于操作session相关方法
     * @returns
     * @memberof ZWBaseCtr
     */
    getSessionSampleObj()
    ...
    /**
     * 一次性加载session到缓存里面
     * @returns {Promise<number>}返回加载了多少个Sesssion
     * @memberof ZWBaseCtr
     */
    ...
    /**
     * 变量方法 注册RPC响应,
     * @param {*} var_obj,需要响应RPC的变量,
     * @param {string} funcname,需要响应的方法,
     * @param {string} type,注册方式:<-> 调用响应,-> 只调用出去,<- 只响应进入
     * @returns {boolean}
     * @memberof ZWBaseCtr
     */
    regRPC( var_obj , funcname ,type )
    {
        if( type == '<->' )
            return this._rpcMgr.regRPC(var_obj,funcname);
        if( type == '->' )
            return this._rpcMgr.regRPCForCall(var_obj,funcname);
        if( type == '<-' )
            return this._rpcMgr.regRPCForResb(var_obj,funcname);
