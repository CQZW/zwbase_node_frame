...
class ZWBaseCtr {
    ...
    async doRouter(req, res, next) {
        ...
        try 
        {
            ...
            {
                //如果是上传文件,先接收文件数据,
                if( ( this.getFuncNameWithPath(param.path) ) == 'ctr_uploadfile' )
                {
    ...
    {
        return zwsession.loadSession( sessionid );
    }
    ...
    async loadAllSessionToCache()
    {
        ...
        let loadcount = await zwsession.loadAllSessionToCache( max,cache );
    ...
    getRouterFunc(param) {
        ...
        let funcname = this.getFuncNameWithPath(param.path);
