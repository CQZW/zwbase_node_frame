...
class ZWBaseCtr {
    ...
    instanceCtr( calssname )
    {
        ...
        if( v == null ) v = new calssname( this._SrvObj );
        return v;
    }
    ...
    async doRouter(req, res, next) {
        ...
        try 
        {
            ...
            await this.dumpSession( param.session );
    ...
    async checkParam(param) {
        ...
        do
        {
            ...
            if ( !await this.checkSession( param.session )  )
            {
                ret.code = 9999;
                ret.msg = '无效的token';
                break;
    ...
    {//创建一个新的session 信息
        let key = new Date().getTime()+'zw' + Math.random()*100000;
        let md5         = crypto.createHash("md5");
        md5.update(key);
        key = md5.digest('hex').toLowerCase();

        let now = new Date();
        let obj = {};
        obj.sessionid = key;
        obj.token = '';
        obj.expire = new Date( now.getTime() + 1000*3600*24 * 7  );
        obj.userinfo = {};
        obj.dumpat = null;
    ...
    /**
     * 开启后台任务,如果是单例后台任务,整个操作系统内,只有一个服务下面的控制器才能执行
     * 比如使用cluster进行负载均衡,会有多个服务实例,如果开启,那么 只有其中一个服务有机会执行任务
     * @param {*} time_ms 默认 10秒
     * @param {bool} isSignle 是否是单例后台任务,默认false,
     */
    startRuningJob(time_ms = 10000 ,isSignle = false) {
        if( isSignle )
        {//如果后台任务需要单例,那么需要进行判断抢占是否可以运行
            this.isCanStartJob().then( (can)=>{
                if( can ) 
                {
                    this.log('get signle run job at pid:',process.pid);
                    this._time_ms = time_ms;
                    setTimeout(()=>{this.job_runing()}, time_ms);
                }
                else 
                {
                    this.log('so not run job for pid:',process.pid);
                }
            });
        }
        else
    ...
    async isCanStartJob()
    {
        return this._SrvObj.ctrRobSingleJobPower();
    }
    /**
     * 所有后台任务,如果需要该控制器逻辑需要有后台任务的, 
     * 当子类任务完成,调用super.job_runing 可以保持任务继续.
     * 如果不调用super.job_running 任务将不会继续执行
     * @memberof ZWBaseCtr
     */
    async job_runing() {
