...
class ZWRouter
{
    ...
    constructor( pathprefix )
    {
        ...
        if( pathprefix[0] !== '/' || pathprefix[pathprefix.length-1] === '/' ) 
            throw new Error('router path invalid,do not like:/xxx/,xxx/,must be /xxx,');
        this._routermap     =   {};
    ...
    clear()
    {
        let vs = Object.values( this._routermap );
        for( let one of vs )
        {
            if( one instanceof ZWRouter ) one.clear();
            else one.setAtRouter( null );
        }
        this._routermap = {};
        this._parentRouter = null;
        this._router.delete( '/*' );
