...
class ZWRPCBridge extends EventEmitter
{
    ...
    stop()
    {
        let vs = Object.keys( this._subscribed_funcid );
        for( let one of vs )
        {
            this._pub_client.unsubscribe( one );
        }
        this._pub_client.unsubscribe( this._bridgerid );

        this._pub_client.quit();
        
        this._out_client.quit();

        //如果还有等待的,直接超时处理了,清空信号事件
        vs = Object.values( this._call_pending );
        this._call_pending = {};
        for( let s of vs )
        {
            s.notify( null );
    ...
    {
        if( this._subscribed_funcid[ funcid ] )
        {//如果已经有人注册过这个方法了,..
            throw new Error('someone has reg this funcid:' + funcid);
        }
        this._pub_client.subscribe( funcid );
        this._subscribed_funcid[ funcid ] = 1;
    }
    /**
     * 我不再响应这个方法
     *
    ...
    iCanNotResbThisFunc( funcid )
    {
        ...
        delete this._subscribed_funcid[ funcid ];
    }
    /**
     * 发送数据到可以响应这个方法的进程,
     * 并且返回对方发送的返回数据
