...
class ZWRPCMgr
{
    ...
    async sendRPC( rpcdata )
    {
        ...
        try
        {
            let callinfo = this._peerprovider.getPeerForCall( rpcdata.func_unique_id );
            if( !callinfo )
            {//如果没有节点响应,那么自己响应
            ...
            if( r )
            {
                ...
                if( r.code == 0 ) 
                {   
                    this._peerprovider.callResult( callinfo.ip,callinfo.callid,true);
    ...
    async _rpc_callout_waper()
    {
        try
        {
            ...
            */
            ...
            return Promise.resolve( rpcdata.errmsg );
        }
        catch(e)
        {
            throw e;
