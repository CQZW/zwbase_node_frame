...
class ZWRPCMgr
{
    ...
    {
        let peer = this._peerprovider.getPeerForCall( rpcdata.func_unique_id );
        if( !peer )
        {//如果没有节点响应,那么自己响应
    ...
    async onRPC( rpcdata )
    {
        ...
        try
        {
            do
            {
                ...
                let { var_name , func_name } = this._getFuncIdInfo( retrpcdata.func_unique_id );
                if( var_name === 'this' )
    ...
    _getFuncIdInfo( funcid )
    {
        ...
        return { var_name: a[1], func_name: a[2] };
    }
    ...
    getDeclareArgName( declare )
    {
        ...
        //参数形式主要是 默认参数,解构参数,剩余参数
        //默认参数包含字符串,数字,对象,函数,箭头函数,解构对象.

        //去除两端的括号
        let exp_br = declare.substr(1,declare.length-2);

        if( exp_br.indexOf( '=' ) == -1 )
        {//如果没有默认参数,那就简单了
            return exp_br;
        }
        //'( a,b="a,c,d",c=( (a,b)=>{ let c=2,d=3; return a==d; }) ,f= function(x){ return x+1})';
        //"(a,{cc:bb} = {cc:'aaaa'},... args )";
        //(a,b,c,d,e,f)
    }
    /**
     * 生成hook函数,截获之前的方法调用
    ...
    _makeHookFunc( orgfunc ,var_name,funcname )
    {
        ...
        let hook_body = "return this.getRPCMgr()._rpc_callout_waper("+arg_names+",'"+var_name+"','"+funcname+"')";//这里调试发现一个现象,funcname 不用 ''框起来,会把函数传递故去
    ...
    async _rpc_callout_waper()
    {
        ...
        if( rpcdata.errcode == 0 )
        {
            ...
            return Promise.resolve( rpcdata.ret_data );//真正的返回值
        }
        else
        {
            //return Promise.reject( retrpcdata.ret_data );
            return Promise.resolve( retrpcdata.ret_data );
        }
    }
