...
class ZWRPCMgr
{
     ...
     * @param {number}     local_pid,需要本地IPC响应的进程ID
     * @returns {Promise<ZWRPCData>} 响应之后返回的数据
     * @memberof ZWRPCMgr
     */
    async sendRPC( rpcdata , local_pid = 1  )
    {
        ...
        try
        {
            do
            {
                ...
                if( callinfo.blocal )//优化
                {//如果选择的是一个本地iP,那么判断下是不是要到指定进程,如果是就直接调用了,不要先HTTP,再IPC
                    let _p = this._peerprovider.getFuncResbPID( rpcdata.func_unique_id );
                    if( _p == 1 )
                    {//如果这个方法没有要求某个进程响应,那就直接http过去,cluster会分配合适的进程响应

                    }
                    else if( _p == process.pid )
                    {//如果这个方法要这个进程响应,那么不需要IPC,也不需要HTTP了
                        retrpcdata = await this.onRPC( rpcdata );
                        break;
                    }
                    else
                    {//如果不是当前进程,但是应该是其他进程,那么直接这里IPC了
                        retrpcdata = await this.sendRPC( rpcdata , _p );
                        break;
                    }
                }
    ...
    async onRPC( rpcdata )
    {
        ...
        try
        {
            do
            {
                ...
                let _pid = this._peerprovider.getFuncResbPID( reginfo.func_unique_id );
                if( _pid > 1 && _pid != process.pid )
                {//如果这个方法需要一个指定的进程响应,并且不是自己,那么通过IPC再次转发
                    return this.sendRPC( rpcdata ,_pid );
                }
    ...
    regRPCForResb( var_obj , funcname )
    {
        if( !funcname ) return false;
        if( typeof funcname == 'function' ) funcname = funcname.name;
        return this._rpcRU( var_obj,funcname,3, process.pid );
    }
     ...
     * @memberof ZWBaseCtr
     */
    _rpcRU( var_obj , funcname , regtype , pid = 1 )
    {
        ...
        {
            //如果已经注册过了
            if( t ) throw new Error('has reged func:',func_id);
            ...
            if( regtype != 2 ) this._peerprovider.iCanResbThisFunc( reginfo.func_unique_id ,pid );
        }
        else
        {
            if( !t ) return true;
