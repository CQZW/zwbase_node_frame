        ...
        try
        {
            do
            {
                ...
                {//如果选择的是一个本地iP,那么判断下是不是要到指定进程,如果是就直接调用了,不要先HTTP,再IPC
                    let _p = this._peerprovider.getFuncResbLocalPID( rpcdata.func_unique_id );
                    if( _p == process.pid )
    ...
    async onRPC( rpcdata )
    {
        ...
        try
        {
            do
            {
                ...
                if( !this._peerprovider.isThisProcCanResb( reginfo.func_unique_id ) )
                {
                    return this.sendRPC( rpcdata , this._peerprovider.getFuncResbLocalPID( reginfo.func_unique_id ) );
                }
