...
class ZWRPCRegInfo
{
    ...
    /**
     * 可以响应该方法的进程 ID,1表示都可以响应,否则需要指定的进程响应
     *
     * @memberof ZWRPCRegInfo
     */
    resb_pid;
    ...
    async onRPC( rpcdata )
    {
        ...
        try
        {
            do
            {
                ...
                if( reginfo.resb_pid != 1 )
                {//如果 这个方法有指明的进程来响应,
                    if( reginfo.resb_pid == process.pid )
                    {//如果正好是这个,那太好了,直接就下面流程就好了

                    }
                    else 
                    {//如果不是当前进程,通过 IPC 进行再次RPC到目标进程
                        let ipc_ret = await this._peerprovider.sendDataIPC( reginfo.resb_pid , rpcdata  )
                        if( ipc_ret.e ) 
                        {
