...
class ZWRPCMgr
{
     ...
     * @param {boolean}     bipc,是否通过IPC发送到本地
     * @returns {Promise<ZWRPCData>} 响应之后返回的数据
     * @memberof ZWRPCMgr
     */
    async sendRPC( rpcdata , bipc = false )
    {
        ...
        try
        {
            do
            {
                if( bipc )
                {
                    call_peer = this._peerprovider.getPeerForCall( rpcdata.func_unique_id ,false );
                    if( !call_peer )
                    {
                        retrpcdata.errcode = 3;
                        retrpcdata.errmsg = 'RPC调用错误:未找到本地响应者';
                        break;
                    }
                    _callid = rpcdata.call_id = call_peer.makeCallId();
                    let ipc_ret = await this._peerprovider.sendDataIPC( call_peer.port , rpcdata  )
                    if( ipc_ret.e )
                    {
                        retrpcdata.errcode  = 3;
                ...
                call_peer = this._peerprovider.getPeerForCall( rpcdata.func_unique_id ,true);
                if( !call_peer )
                {
                    retrpcdata.errcode = 3;
        ...
        if( call_peer )
        {
            call_peer.callEnd( _callid , retrpcdata.errcode == 0 );
            this._peerprovider.hangUp( call_peer );
        }
        return Promise.resolve( retrpcdata );
    }
    ...
    async onRPC( rpcdata )
    {
        ...
        try
        {
            do
            {
                ...
                //如果本当前进程无法响应,发给其他进程
                if( !this._peerprovider.isThisProcCanResb( rpcdata.func_unique_id ) )
                    return this.sendRPC( rpcdata , true );
