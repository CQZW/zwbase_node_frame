...
class ZWBaseSrv
{
    ...
    async srvWillStart()
    {
        ...
        { 
            ...
            if( !once )
            {//一次性的操作,这样操作是因为通常ctr都有一个共同的基础类,可以做这种操作
                if( this._cfg.need_session ) await ctr.loadAllSessionToCache();
                once = true;
            }
        }
    ...
    async job_runing()
    {
        if( this._cfg.need_session )
        {
            //缓存里面所有的session 尝试入库
            await this.job_session();
        }
        setTimeout(()=>{this.job_runing()}, this._cfg.time_ms );
    }
    /**
    ...
    async job_session()
    {
        ...
        for( let one of all )
        {
            ...
            if( !one.isVaild() && await one.delSession() ) d++;
        }
        logger.log('dump sesssion:',i,' del session:',d);
        return Promise.resolve();
    }
