const   express     = require('express');
const   logger      = require('./zwlogger')

const   https       = require('https');
const   http        = require('http');
const   dgram       = require('dgram');
const   LRU         = require("lru-cache");


class ZWBaseSrv
{
    constructor()
    {
        this.httpsapp       = express();
        this.httpapp        = express();
        this._DBObj         = null;//持有的数据库对象
        logger.cfgFor4JS();

        //是否启用https
        this.needhttps      = true;
        this._cfg           = {
            domain:'127.0.0.1',
            resentry:'/public/',//访问资源文件的入口
            reslocal:'public',      //资源文件真正的目录
            httpport:80,
            httpsport:443,
            signle_job_lock_port:5005,//独占任务端口,
            maxcache:100000,
            time_ms:10000//服务后台任务默认10秒一次
        };

        //所有的路由器对象
        this._routers = [];

        //是否抢占单例任务成功
        this._signle_locked = undefined;

        this.srvConfig();
        
        //缓存
        this._cache = new LRU( this._cfg.maxcache );

    }
    
    /**
     * 继承修改服务配置
     * 
     * @returns {},配置map
     * @memberof ZWBaseSrv
     */
    srvConfig()
    {
        return this._cfg;
    }
    
    /**
     * 获取cache
     * 自定义,memcache,或者redis之类的 实现get/set方法即可
     * @returns
     * @memberof ZWBaseSrv
     */
    ctrGetCache()
    {
        return this._cache;
    }
    /**
     * 给ctr调用
     * 获取数据库对象
     * @returns
     * @memberof ZWBaseSrv
     */
    ctrGetDB()
    {
        return this._DBObj;
    }
    //或者服务站点配置等详细信息.比如域名,什么之类的
    /**
     * 给ctr调用
     * 获取配置对象
     * @returns {}
     * @memberof ZWBaseSrv
     */
    ctrGetSrvCfgInfo()
    {
        return Object.assign( {}, this._cfg );
    }

    /**
     *  给ctr调用
     * 获取整个服务的所有路由对象
     * @returns []
     * @memberof ZWBaseSrv
     */
    ctrGetSrvRouters()
    {
        return this._routers
    }

    /**
     * 抢占单例任务权限,
     * fork,cluster等方式可以运行多个服务实例,
     * 但是,有些任务只能单例运行的,就需要挑选其中一个 服务来执行,
     * 那么进行端口抢占,如果抢占到了,这个操作系统里面只能有一个服务有机会执行任务
     * @memberof ZWBaseSrv
     */
    async ctrRobSingleJobPower()
    {
        if( this._signle_locked != undefined )
        {//已经抢占过了,那么直接返回结果,
            return Promise.resolve( this._signle_locked );
        }
        return new Promise( (resolve,reject)=>{
            const s = dgram.createSocket('udp4');
            s.on( 'error',(err)=>{
                //if( err.code == 'EADDRINUSE' ) resolve( false );
                this._signle_locked = false;
                resolve( false );
            });
            s.bind( {   address: 'localhost',
                        port: this._cfg.signle_job_lock_port,
                        exclusive: true } ,()=>{
                            this._signle_locked = true;
                            resolve(true);
                        });
        });
    }

    /**
     * 内部方法,获取express对象
     * @returns
     * @memberof ZWBaseSrv
     */
    getApp()
    {
        if( this.needhttps ) return this.httpsapp;
        return this.httpapp;
    }
    
    /**
     * 配置 Express 中间件
     * @memberof ZWBaseSrv
     */
    cfgExpress()
    {
        //支持的content-type...
        // parse application/x-www-form-urlencoded
        this.getApp().use(express.urlencoded({limit:5242880, extended: false }));
        // parse application/json
        this.getApp().use(express.json({limit:5242880}));

        //处理xml 如果要处理 微信支付回调 需要这个
        this.getApp().use(express.raw({limit:5242880,type:'text/xml'}));

        this.getApp().use(logger.connectLogger());

        //公开文件,资源文件等等
        this.getApp().use( this._cfg.resentry ,express.static( this._cfg.reslocal ,
        {
            cacheControl:true,
            maxAge:(1000*3600*24)
        }));
    }

    /**
     * 配置路由
     * 
     * 继承添加路由配置 必须调用在最后调用super
     * @param {[]} routers [ZWRouter]
     * @memberof ZWBaseSrv
     */
    cfgRouter( routers = [] )
    {//express是按顺序使用路由的,最后都会到这里,继承必须调用super,到这里
        //关于路由,见这里说明
        //http://www.expressjs.com.cn/4x/api.html#router
        //比如这种...
        //app.use( '/api/v1' ,routerapiv1 );
        //app.use( '/api/v2' ,routerapiv2 );

        for( let r of routers )
        {
            this.getApp().use( r.getPathPrefix(),r.getRouter() );
            this._routers.push( r );
        }

        //如果路由 走到这里,说明之前设置的路由全部都没有接住,,那么最后这3个处理下了,否则就失败了
        this.getApp().all('/',this.r_root);
        this.getApp().all('*',this.r_404);
        this.getApp().use(this.r_500);

    }

    createHttp()
    {
        this._http_forlisten =  http.createServer(this.httpapp);
        if( this.needhttps )
        {
            //默认情况把所有http的流量都转到 https,自己实现一个跳转方法

            this.httpapp.all('*',(req,res,next) => {
                this.r_http_redirect(req,res,next);
            });

            this._https_forlisten = https.createServer(this.getHttpsOptions(), this.httpsapp);
        }
        logger.log('start http srv ok');
    }

    /**
     * 开始监听端口,开始服务,
     * 放到后面,是等配置路由都搞定了,才开始服务
     * @memberof ZWBaseSrv
     */
    listenHttp()
    {
        if( this._http_forlisten ) this._http_forlisten.listen( this._cfg.httpport );
        if( this._https_forlisten ) this._https_forlisten.listen( this._cfg.httpsport );
    }
    
    /**
     * 继承修改,如果需要https,
     * http证书配置,
     * @returns opt{ key cert ca }
     * @memberof ZWBaseSrv
     */
    getHttpsOptions()
    {
        logger.error('get your key cfg for https');
        /*
        let options = {};
        options.key = fs.readFileSync( './xxx.com.key' );
        options.cert = fs.readFileSync( './xxx.com.cer' );
        options.ca = fs.readFileSync( './xxxx.com_ca.crt' );
        return options;
        */
        return null;
    }
    
    /**
     * 启动服务
     * @returns
     * @memberof ZWBaseSrv
     */
    async start()
    {
        try
        {

            await this.ctrRobSingleJobPower();

            this._DBObj = await this.startDB();

            this.createHttp();

            this.cfgExpress();

            this.cfgRouter();

            await this.srvWillStart();

            this.listenHttp();

            this.srvDidStarted();

            return Promise.resolve();
        }
        catch( error)
        {
            logger.error('start srv failed,',error);
            process.exit( 101 );
        }
    }

    /**
     * 获取一个控制器,用于处理通用的操作
     * 因为通常会有basectr,通用的逻辑在这里,所以随便获取一个即可
     * @returns
     * @memberof ZWBaseSrv
     */
    getOneCtr()
    {
        //加载所有的缓存到缓存,加载一次就够了
        for( let one of this._routers )
        {
            let t = one.getAllCtrs();
            for( let ctr of t )
            {
                return ctr;
            }
        }
        return null;
    }
    async srvWillStart()
    {
        //启动后台任务
        this.startRuningJob();

        //加载缓存到cache里面
        this.getOneCtr().loadAllSessionToCache();

        return Promise.resolve( );
    }
    /**
     * 处理完成之后必须调用super,
     * 服务启动成功之后默认会通知所有ctr.srvStartOk
     */
    srvDidStarted()
    {
        for( let one of this._routers )
        {
            let t = one.getAllCtrs();
            for( let ctr of t )
            {
                ctr.srvStartOk();
            }
        }
    }

    /**
     * 继承添加启动数据库操作
     * 启动数据库
     * @memberof ZWBaseSrv
     */
    async startDB()
    {
        logger.error('start your db at you subclass');
        throw new Error('please start db frist');
    }
    //10秒执行一次任务
    startRuningJob( time_ms )
    {
        if( time_ms ) this._cfg.time_ms = time_ms;
        this.ctrRobSingleJobPower().then( (bsingle) => {
            setTimeout(()=>{this.job_runing(bsingle)}, this._time_ms );
        });
    }
    /**
     * 子类继承之后,调用super可以继续执行,否则不会继续执行任务
     *
     * @param {*} isSingle-当前任务是不是本机器唯一任务
     * cluster可以多进程启动任务,所以每个进程有自己的任务,需要需要机器唯一参考这个值
     * 如果要全局唯一,修改 ctrRobSingleJobPower 方法,加锁即可
     * @returns
     * @memberof ZWBaseSrv
     */
    async job_runing( isSingle )
    {
        //加载所有的缓存到缓存,加载一次就够了
        let all = this.ctrGetCache().values();
        let onectr = this.getOneCtr();
        for( let one of all )
        {
            await onectr.dumpSession( one );
        }
        setTimeout(()=>{this.job_runing( this._signle_locked )}, this._cfg.time_ms );
        return Promise.resolve();
    }

    /**
     * 可继承修改
     * 路由 根目录,
     * @param {*} req
     * @param {*} res
     * @param {*} next
     * @memberof ZWBaseSrv
     */
    r_root(req,res,next)
    {
        res.send('Hello...');
    }

    /**
     * 可继承修改
     * 找不到路由情况,404处理
     * @param {*} req
     * @param {*} res
     * @param {*} next
     * @memberof ZWBaseSrv
     */
    r_404( req,res,next)
    {
        logger.log( 'not find any router' );
        res.status(404).send('not find');
    }

    /**
     * 可继承修改,
     * 服务器有错误处理
     * @param {*} err
     * @param {*} req
     * @param {*} res
     * @param {*} next
     * @memberof ZWBaseSrv
     */
    r_500(err,req,res,next)
    {
        logger.log( 'srv has error:' ,err);
        res.status(500).send('srv has error');
    }
    /**
     * 可继承修改
     * http直接重定向到哪儿
     * @param {*} req 
     * @param {*} res 
     * @param {*} next 
     */
    r_http_redirect(req,res,next)
    {
        logger.log('cfg your http redir');
        res.redirect( 'https://' + this._cfg.domain + '/' );
    }
}

module.exports = ZWBaseSrv;