...
class ZWBaseSrv
{
    ...
    async robGlobalLock()
    {
        if( !this._cache_redis_client ) return Promise.resolve(false);
     ...
     * @memberof ZWBaseSrv
     */
    startRuningJob( time_ms , firenow = false )
    {
    ...
    _runjob()
    {
        this._job_timer = setTimeout(()=>{this.job_runing()}, this._cfg.time_ms );
    }
