...
class ZWBaseSrv
{
    ...
    {
        if( this.canRPC && forceRob ) logger.log('maybe has some error');
    ...
    createHttp()
    {
        if( this.needhttps == 0 || this.needhttps == 2 )
    ...
    async srvWillStart()
    {
        ...
        if( this.canRPC )
        {
            this._rpcpeermgr = new ZWPeerMgr( this.machine_lock_socket ,
                this.machine_lock_socket?this.machine_lock_socket.address().port:this._cfg.machine_lock_port );
            if( !await this._rpcpeermgr.start() ) throw new Error('start peer mgr failed');
        }
