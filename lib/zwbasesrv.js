...
class ZWBaseSrv
{
    ...
    async _releaseGlobalLock()
    {
        ...
        if( this.global_lock ) this._cache_redis_client.del( this._global_lock_key );
    }
    /**
     * 获取本机IP地址,
     *
    ...
    async robGlobalLock()
    {
        ...
        return new Promise((resolve,reject) => {
            this._cache_redis_client.setnx( this._global_lock_key , this._global_lock_val ,(e,r)=>{
                this.global_lock = r;
                if( r )
                {
                    this._cache_redis_client.expire( this._global_lock_key, this._cfg.lock_timeout );
