...
class ZWQPSCtr
{
    ...
    async checkQPS()
    {
        ...
        {//间隔太小了,,,睡眠下,
            await this.sleep( this._min_req_gap - gap );
        }
...
class ZWJobArrayThread
{
    constructor( threadfunc )
    {
        this._threadfunc = threadfunc;
    }
    cfgInfo()
    {
        this._jobarrmax = 1000;
        this._waitms    = 10000;//等待任务超时时间
        this._loopgapms = 1000;//循环间隔时间
    }
    ...
    async _loop()
    {
        try
        {
            ...
            while(1)
            {
                let jobinfo = await this._signalobj.wait( this._stop?1:this._waitms );
                //如果没有数据,就是等待超时了,那么就下次循环了
                if( !jobinfo ) break;
        ...
        {//要停止了,
            ...
            {
                this._stopsingal.notify(true);
            }
        }
        else 
            setTimeout( ()=>{this._loop(),this._loopgapms} );
    }
    /**
     * 添加一个任务
     *
