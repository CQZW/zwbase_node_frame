...
class ZWPeerMgr  extends EventEmitter
{
    ...
    async _broadcastmyself()
    {
        ...
        {//广播给其他节点,需要将整个机器响应的方法都告诉别人
            ...
            return this._sendData( ZWPeerMgr.st_cmd_post_peer_to_all , this._post_to_remote_peer );
        }
    ...
    getAllPeers()
    {
        return Object.values( this._remote_peers );
    }
    /**
     * 删除节点
     *
    ...
    {
        let r = {};
        let r_port =  this._getProcPortByPID( pid );
    ...
    _getProcPortByPID( pid )
    {
        let a = Object.keys( this._local_peers );
        for( let port of a )
        {
            if( this._local_peers[ port ].pid == pid ) return port;
        }
        return 0;
    }
