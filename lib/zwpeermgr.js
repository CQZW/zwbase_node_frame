...
class ZWPeerMgr  extends EventEmitter
{
            ...
            do
            {
                ...
                {//收到了广播节点的数据
                    ...
                    {//直接删除
                        ...
                        delete this._callstartat[recvdata.ip];
    ...
    /**
     * 获取一个方法响应的进程ID
     * 并且表明这个方法是否为本机进程都响应的方法
     * @param {string} funcid
     * @returns {number,boolean}
     * @memberof ZWPeerMgr
     */
    getFuncResbLocalPID( funcid )
    {
        /**
         * @type Array<ZWPeer>
         */
        let a = Object.values( this._local_peers );
        /**
         * @type ZWPeer
         */
        let p = null;
        ...
        for( let one of a )
        {
            if( !one.funcIDs[funcid] ) {b_all = false;continue;}//不响应的不要
            if( p == null ) {p = one;continue;}
            //优先取调用次数更小的,就是说更闲的
            if( p._allST.call_count < one._allST.call_count ) p = one;
        }
        if( !p ) return {pid:-1,ball:false};
         return {pid:p.funcIDs[ funcid ],ball:b_all};
    }
    /**
     *当前进程是否可以响应者请求,
     *
     ...
     * @memberof ZWPeerMgr
     */
    getRemotePeerForCall( funcid ,label = null )
    {
        let ks = Object.keys( this._remote_peers );
    ...
    callResult( ip, callid , bok )
    {
        ...
        //所以 getRemotePeerForCall callResult配对使用
        delete this._callstartat[ip][callid];
         ...
         * @type {ZWPeer}
         */
        let peer = this._remote_peers[ip];
    ...
    async sendDataIPC( pid , data , subtype = 0 )
    {
        ...
        let _tpeer = this._getLocalPeerByPID( pid );
        if( _tpeer == null )
        {
            r.e = '未找到IPC响应者';
            return Promise.resolve( r );
        }
        let r_port = _tpeer.port;
        if( isNaN( r_port) || r_port <= 0 )
        {
            r.e = 'IPC响应者端口异常';
            return Promise.resolve( r );
        }
     ...
     * @returns {ZWPeer}
     */
    _getLocalPeerByPID( pid )
    {
        let a = Object.keys( this._local_peers );
        for( let port of a )
        {
            if( this._local_peers[ port ].pid == pid ) return this._local_peers[port];
        }
        return null;
    }
