...
class ZWPeerMgr
{
    ...
    constructor( main_socket, mainport , peerlabel = '' )
    {
        ...
        if( this._isMain )
        {
            ...
            if( this._self.port != this._main_port ) throw new Error('main port not eq main socket port');
        }
        else
        {
    ...
    callResult( ip, callid , bok )
    {
        ...
        if( !at ) return;
        delete this._callstartat[ip][callid];//如果有大量接口没有执行 callResult,这里很多...
    ...
    async broadcastDel( peer )
    {
        ...
        if( this._isMain )
        {
            return new Promise( (resolve,reject) => {
                let msg = JSON.stringify( peer );
                this._socket.send( msg , this._main_port , '255.255.255.255',(err)=>{
                    logger.log('broadcastDel ok');
                    resolve(err==null);
                });
            });
        }
        else
        {
            return new Promise( (resolve,reject) => {
                ...
                tmpsocket.send( msg , this._main_port , '255.255.255.255',(err)=>{
                    logger.log('broadcastDel ok');
                    resolve(err==null);
                });
            });
    ...
    _recvMsg(msg,rinfo)
    {
        try
        {
            do
            {
                ...
                if( !this._isMain )
                {//从进程只能收到这个消息
                    if( recvobj.peerlist  ) this._recvMainPeerList( recvobj.peerlist );
                    //扩展..
                    break;
                }
                //收到从进程消息,就回复全部节点给他
                if( recvobj.iam === 'slave')
                {
                    if( rinfo.address === this._localip )
                    {
